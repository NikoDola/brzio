...
`separatePenetrations` resolves circle contacts after every physics step using up to three sweeps. It corrects 40% of overlap beyond a 2px tolerance per sweep, split by inverse mass, and respects the container walls and floor. Rechecking neighbours lets crowded pockets settle. Position corrections preserve velocity; only approaching motion along the contact normal is removed, so sliding and separating bounces survive. Small corrections leave sleeping planets asleep. Deep corrections (excess above 20% of the smaller radius) wake the affected bodies and stop their spin. The old 10px allowance plus repeatedly waking and zeroing both bodies trapped the Moon inside Mars in Scenario 2 and kept the pile moving. Do not restore that combination. Planets already outside the open rim are not clamped back into the container.
...
Two circle-specific safety nets are documented above: `separatePenetrations` relaxes overlaps while preserving separating motion and sleep, and an `afterUpdate` hook damps and caps every planet's angular velocity.
...
- **Baked bitmaps self-heal after GPU context loss.** A GPU process reset (driver crash, laptop sleep, memory pressure) wipes every offscreen canvas, which used to leave the ship and all planets permanently invisible while procedural drawing kept working. renderer.js retains the source Images, checks a baked bitmap's centre pixel on a slow timer (and when the tab becomes visible), and re-bakes everything if it reads transparent. Keep the retained-image arrays in sync when adding new baked sprites.
